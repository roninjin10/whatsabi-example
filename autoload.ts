...
const provider = env.INFURA_API_KEY
    ? new ethers.InfuraProvider("homestead", env.INFURA_API_KEY)
    : ethers.getDefaultProvider(env.NETWORK || "homestead");
...
async function main() {
    ...
    while (true) {
        const [abi, unresolved] = partitionBy(
            r.abi,
            (a) => a.type !== "function" || "name" in a,
        );
