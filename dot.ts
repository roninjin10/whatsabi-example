...
// TODO make this work outside of the whatsabi repo
import { Program, Function } from "@shazow/whatsabi";
// TODO make this work outside of the whatsabi repo
import { disasm } from "@shazow/whatsabi";
// TODO make this work outside of the whatsabi repo
import { mnemonics } from "@shazow/whatsabi";
// TODO make this work outside of the whatsabi repo
import { defaultSignatureLookup } from "@shazow/whatsabi";
// TODO make this work outside of the whatsabi repo
import { bytesToHex } from "@shazow/whatsabi";
...
export function* programToDotGraph(
	p: Program,
	...
	const nameLookup = Object.fromEntries(
		Object.entries(p.selectors).map(([k, v]) => [v, k]),
	);
	...
	yield "\t\trankdir=LR;";
	yield `\t\t${Object.keys(p.selectors)
		.map((s) => '"' + s + '"')
		.join(" ")}`;
	yield "\t}";

	const jumps: Function[] = Object.values(p.selectors).map(
		(j) => p.dests[j],
	) as Function[];
...
async function main() {
	...
	for (const sel of selectors) {
		...
		const sigs = await withCache(`${sel}_selector`, async () => {
